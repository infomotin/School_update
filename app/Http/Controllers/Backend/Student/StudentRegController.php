...
class StudentRegController extends Controller
{
    public function StudentRegView(){
    	...
    	$data['class_id'] = StudentClass::first()->id;
    	// dd($data['class_id']);
    ...
    public function StudentRegStore(Request $request){
    	...
    	DB::transaction(function() use($request){
			...
			$student = User::where('usertype','Student')->orderBy('id','DESC')->first()->id;
				...
				foreach($request->discount as $key => $item){
						if($item > 0){
							$discount_student = new DiscountStudent();
							$discount_student->assign_student_id = $assign_student->id;
							$discount_student->fee_category_id = $key+1;
							$discount_student->discount = $item;
    ...
    public function StudentRegEdit($student_id){
		...
		$discount = $data['editData']['discount'];
		$data['addmission_discount'] = @(collect($discount)->where('fee_category_id',1)->first()->discount);
		$data['tuition_discount'] = @(collect($discount)->where('fee_category_id',2)->first()->discount);
		$data['exam_discount'] = @(collect($discount)->where('fee_category_id',3)->first()->discount);
 ...
 public function StudentRegUpdate(Request $request,$student_id){
    	...
    	DB::transaction(function() use($request,$student_id){
			...
			foreach($request->discount as $key => $item){
				if($item > 0){
					DiscountStudent::updateOrCreate(['assign_student_id' => $assign_student->id, 'fee_category_id' => $key+1],['discount' => $item]);
				}
			}
